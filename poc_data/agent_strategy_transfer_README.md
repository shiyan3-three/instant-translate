...
```powershell
.\.venv\Scripts\python.exe poc_agent_strategy_transfer.py
```
...
```powershell
.\.venv\Scripts\python.exe poc_agent_strategy_transfer.py --dry-run
```
...
.\.venv\Scripts\python.exe poc_agent_bootstrap_review.py `
  ...
  --results logs\poc\agent-strategy-YYYYMMDD-HHMMSS.jsonl `
  --review logs\poc\agent-strategy-blind-YYYYMMDD-HHMMSS.jsonl
```

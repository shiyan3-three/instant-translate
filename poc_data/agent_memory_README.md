...
```powershell
.\.venv\Scripts\python.exe poc_agent_memory.py `
  --baseline logs\poc\session-replay-20260630-143244.jsonl
```
...
.\.venv\Scripts\python.exe poc_agent_bootstrap_review.py `
  ...
  --results logs\poc\agent-memory-YYYYMMDD-HHMMSS.jsonl `
  --review logs\poc\agent-memory-blind-YYYYMMDD-HHMMSS.jsonl
```

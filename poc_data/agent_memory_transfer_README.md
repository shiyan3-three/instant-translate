...
数据文件：`poc_data/agent_memory_transfer_dataset.json`
...
```powershell
.\.venv\Scripts\python.exe poc_agent_memory_transfer.py
```
...
```powershell
.\.venv\Scripts\python.exe poc_agent_memory_transfer.py --dry-run
```
...
```powershell
.\.venv\Scripts\python.exe poc_agent_memory_transfer_review.py `
  --results logs\poc\memory-transfer-YYYYMMDD-HHMMSS.jsonl `
  --review logs\poc\memory-transfer-blind-YYYYMMDD-HHMMSS.jsonl
```

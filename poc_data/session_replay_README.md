...
```powershell
.\.venv\Scripts\python.exe poc_session_replay.py
```
...
.\.venv\Scripts\python.exe poc_agent_bootstrap_review.py `
  ...
  --results logs\poc\session-replay-YYYYMMDD-HHMMSS.jsonl `
  --review logs\poc\session-replay-blind-YYYYMMDD-HHMMSS.jsonl
```
...
```powershell
.\.venv\Scripts\python.exe poc_session_replay.py --dry-run
```
...
```powershell
.\.venv\Scripts\python.exe poc_session_replay.py `
  --transcript logs\poc\session-replay-transcript-YYYYMMDD-HHMMSS.json
```

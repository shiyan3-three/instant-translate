...
```powershell
.\.venv\Scripts\python.exe poc_reference_injection.py --validate-only
```

Inspect exact request payloads without spending API quota:

```powershell
.\.venv\Scripts\python.exe poc_reference_injection.py --allow-draft --dry-run
```

The dry run writes exact request payloads to `logs/poc/*.jsonl`.  It never

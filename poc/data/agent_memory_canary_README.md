...
```powershell
.\.venv\Scripts\python.exe -m poc.poc_agent_memory_canary `
  --baseline poc\results\agent-memory-20260701-185140.jsonl `
  --memory poc\results\agent-memory-state-20260701-185140.json
```
...
.\.venv\Scripts\python.exe -m poc.poc_agent_bootstrap_review `
  ...
  --results poc\results\agent-memory-canary-YYYYMMDD-HHMMSS.jsonl `
  --review poc\results\agent-memory-canary-blind-YYYYMMDD-HHMMSS.jsonl
```

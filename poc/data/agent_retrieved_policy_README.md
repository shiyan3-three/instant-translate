...
```powershell
.\.venv\Scripts\python.exe -m poc.poc_agent_retrieved_policy
```

脚本自动读取 `poc/results` 中最新的正式 `agent-strategy-*.jsonl` 基线。也可以显式指定：

```powershell
.\.venv\Scripts\python.exe -m poc.poc_agent_retrieved_policy `
  --baseline poc\results\agent-strategy-20260702-212448.jsonl
```
...
.\.venv\Scripts\python.exe -m poc.poc_agent_bootstrap_review `
  ...
  --results poc\results\agent-retrieved-policy-YYYYMMDD-HHMMSS.jsonl `
  --review poc\results\agent-retrieved-policy-blind-YYYYMMDD-HHMMSS.jsonl
```
